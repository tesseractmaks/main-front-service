...
export async function slDetailFeatures(detailData, ids = "sl-slider", classSl = "block-slider") {

    let divslider = document.createElement("div")
    divslider.setAttribute("id", ids)

    divslider.classList.add(classSl)
...
export function slowSliderEstate() {
	...
	if (counterSlowEstate >= images.length) {
		counterSlowEstate = 0
	}
	images[counterSlowEstate].classList.add("active-image")
